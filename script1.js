...
function updateTable(data) {
    ...
    data.forEach((row) => {
      const contact = row.contact || 'Your Organization Name';
      const newRow = `
        <tr>
          <td>${row.id}</td>
          <td>${row.item}</td>
  ...
  function fetchDataTwo() {
    $.ajax({
      url: "display_orgs.php", // Replace "fetch_data.php" with the actual PHP file name
      dataType: "json",
      success: function (data) {
        updateOrgs(data);
      },
